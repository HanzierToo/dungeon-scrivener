...
- Mira and Rowan are separate entity speakers with typed entity state. Mira's conversation is interrupted by Rowan's, then resumed. Conditional Rowan dialogue requires trust >= 1. Dialogue selections are `PlayerInput` values containing conversation, line, and option IDs. Rowan's secret option history remains available after the conversation is completed and is used by Mira's later silver-key condition.
- The show-key option exercises `has-item` against world inventory and remains visible but disabled when the key is absent, with a locale-resolved authored disabled reason. Dialogue history is a chronological saved ledger and the compatible save retains entries after both conversations complete.
...
- Engine setup compiles all three declarations into one `CompiledScriptBundle`, then calls `createGameEngine(host, bundle)` with the injected `ScriptExecutorApi`. `createSession(projectId, world, options)` verifies exact IDs/paths/languages/entrypoints against world.json; script-capable later calls repeat that check against their supplied world. The timed rule's `run-script(echo-check)` effect synchronously calls `executeScript` at its effect-list position. Later rules in the same cascade invoke `keeper-check` and `witness-check` through the same executor. The executor receives only IR, origin, bounded remaining limits, and capability callbacks; it does not receive or mutate a session snapshot.
...
World randomness is unseeded, so the host supplies an entropy source and no seed. The starting session has currentNodeId taproom, trust 0, bell-rung false, bell-echoes 0, return-visits 0, empty runtime tags and inventory, `nextInventoryStackOrdinal` 0, gameTimeMilliseconds 0, and no random outcomes. nodeVisitCounts.taproom begins at 1. Entity state starts with Mira disposition welcoming and Rowan disposition guarded.
...
| 9 | `dialogue-option` (rowan-story, rowan-first, tell-secret) | Accepted; records the selected option, advances to rowan-secret, then runs the option effects. | history(option-selected:rowan-story/tell-secret) → line(rowan-first → rowan-secret) → history(line-seen:rowan-secret) |
| 10 | `dialogue-option` (rowan-story, rowan-secret, return-to-mira) | Records the option. The terminal Rowan conversation completes before its authored resume effect restores Mira at mira-after-return. | history(option-selected:return-to-mira) → complete(rowan-story) → effect(resume-conversation:mira-story) → history(line-seen:mira-after-return) |
| 11 | `dialogue-option` (mira-story, mira-after-return, claim-silver-key) | The trust, Rowan line-history, and Rowan option-history predicates are true. Mira advances; option effects add one silver key. | history(option-selected:claim-silver-key) → line(mira-after-return → mira-farewell) → history(line-seen:mira-farewell) → effect(add-item:world/silver-key/1) → inventory(+silver-key×1) |

The save samples capture the state after step 11 with the chronological dialogue ledger intact, both conversations completed, `stack-0` holding the silver key, and `nextInventoryStackOrdinal` set to 1. A further inventory request is `{ "kind": "inventory", "operation": { "kind": "transfer", "stackId": "stack-0", "quantity": 1, "destination": { "owner": { "kind": "entity", "ownerId": "traveler" } } } }`; an accepted transfer moves the same full-stack ID to traveler ownership. Use, equip, and unequip use the same `inventory` input wrapper with operation fields `{kind:"use",stackId}`, `{kind:"equip",stackId,slotId}`, and `{kind:"unequip",stackId,slotId}`. Invalid stack, owner, container, quantity, or slot references reject atomically and consume no action time.
