...
Status: **Not accepted.** The latest Playwright run passes the complete direct-open flow in Chromium and Firefox, including the save ZIP download/import round-trip. Playwright WebKit still fails before the main `file://` document loads. The user-provided Safari screenshots show that Safari opened the page and loaded its local PNG/WAV, but they do not show the full action and save round-trip or enough request detail to close the WebKit evidence gap. Do not start 1B or 1C until that gap is resolved.
...
`spikes/direct-open/evidence/browser-results.json` is the corrected rerun. The initial report is preserved as `spikes/direct-open/evidence/browser-results-initial.json`.
