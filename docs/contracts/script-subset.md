...
Scripts are reachable only through an authored `run-script` effect that names a script declared in `world.scripts`. A run-script effect may appear in choice, command, dialogue-option, lifecycle, or rule effects. It invokes the named script's `main` synchronously at that position in the containing effect list. Each `api.request` or `api.emit` call is validated and applied to the provisional action transaction immediately in script execution order, before the next authored effect runs. Events enter the shared FIFO queue; matching rules run under the data-model priority and ordering rules. A script cannot invoke a script directly through the capability API. A rule triggered by a script-emitted event may invoke another script and is part of the same top-level action.
...
Seeded random calls use the session PRNG and update the persisted PRNG state only on commit. Unseeded calls use the engine host's injected uint32 entropy source; each accepted result is recorded with ordinal, source script ID, operation, bounds, and value. Saves persist those unseeded records for the action trace/replay contract. No seed is required or stored in unseeded mode.
