...
The save records project ID, game version, engine version, playable-content fingerprint, slot metadata, saved time, and the entire resumable session: current node, world/node/entity state, node visit counts, conversation stack and history, game time, seeded RNG state, logged unseeded outcomes, once-only rule guards, and inventory stacks. A save importer validates the entire archive and payload before replacing the active session.

Compatibility requires exact equality of `projectId`, `gameVersion`, `engineVersion`, and `contentFingerprint`. Any mismatch rejects the save with a diagnostic and leaves the active session unchanged. Saves move between hosted and exported copies when all four values match. A save slot count limits in-app slots only; it cannot prevent the player from copying ZIP files. Save operations are explicit user downloads and imports.
...
The `playable-files-v1` set contains `project.json`, `world.json`, every accepted file under `locales/`, every script file declared by `world.json`, every managed asset referenced by the world's Markdown or locale content, and the optional player CSS file named by world settings. Resolve each reference to its accepted VFS path before hashing. Include each path once. A missing, unsafe, or conflicting reference is a model diagnostic and cannot produce a successful fingerprint.
