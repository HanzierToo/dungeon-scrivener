...
The save records project ID, game version, engine version, playable-content fingerprint, slot metadata, saved time, and the entire resumable session: current node, world/node/entity state, mutable entity tags, node visit counts, nullable active conversation, suspended conversation stack, durable dialogue history, game time, randomness mode, initial seed and current PRNG state, complete unseeded random outcome history, clock baseline/activity, once-only rule guards, and inventory stacks with stable stack IDs and `nextInventoryStackOrdinal`. `dialogueHistory` preserves ordered line-seen and option-selected entries across interruption, resume, and conversation completion. `randomInitialSeed` is the normalized seed chosen at session creation; `randomSeed` is the current xorshift32 state. Both are uint32 in seeded mode and null in unseeded mode. The current state resumes future seeded draws exactly. The ordered unseeded history supports call-verified replay and is never truncated. Mutable tags are saved as entity ID to sorted unique tag arrays, including unchanged authored tags, so save/load does not need to reconstruct runtime tag state from a potentially changed project. A save importer validates the entire archive and payload before replacing the active session.

This is a pre-release correction to the v1 schema. The schemaVersion remains explicit and is still required to equal 1. Earlier 1B draft saves without `dialogueHistory`, `activeConversation`, stack IDs, or `nextInventoryStackOrdinal` are rejected as malformed; no migration is defined for those drafts. The fixture saves are updated to the corrected shape. Once v1 ships, any incompatible save change requires a separately versioned migration contract.

The required `gameVersion` in `project.json` is the only author-controlled game version. Save creation copies `manifest.gameVersion` into the save; there is no second author setting in the world or save-compatibility target. The target supplies the current manifest, engine version, and recomputed playable-content fingerprint. Compatibility requires exact equality of save project ID and target manifest project ID, save game version and target manifest game version, engine version, and content fingerprint. `checkSaveCompatibility` returns every mismatched field key from `projectId`, `gameVersion`, `engineVersion`, and `contentFingerprint`. If any field differs, the caller rejects the load and leaves the active session unchanged. There is no automatic migration of player saves in v1. Saves move between hosted and exported copies when all four values match. A save slot count limits in-app slots only; it cannot prevent the player from copying ZIP files. Save operations are explicit user downloads and imports.

An exported player's `SaveCompatibilityTarget` is created at export time from the accepted manifest, the exact accepted `ContentFingerprint.digest`, and the `engineVersion` embedded in its prebuilt player artifact. The generated runtime uses that same target for every save compatibility check and writes those same values into new saves; it does not recompute a different fingerprint or substitute the host app's engine version.
