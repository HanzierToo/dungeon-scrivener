...
export function Player({ view, onInput, startupFailure, themeCss, mediaAssets, locales = [], onLocaleChange, onLoad, onOpenSettings, titleContent, settingsContent }: PlayerProps) {
  ...
  return <div className="ds-player" data-project-id={view.projectId}>
    ...
    <div className="ds-player__layout">
      <main className="ds-player__main">
        <article className="ds-player__scene" aria-labelledby="ds-player-scene-title"><h2 id="ds-player-scene-title">{view.currentNode.title}</h2><div className="ds-player__content">{view.currentNode.blocks.map((block, index) => renderBlock(block, `block-${index}`, mediaAssets))}</div></article>
...
function renderBlock(block: SafeBlock, key: string, mediaAssets?: MediaAssetApi): ReactNode {
  switch (block.kind) {
    case 'paragraph': return <p key={key}>{block.children.map((child, i) => renderInline(child, `${key}-${i}`, mediaAssets))}</p>;
    case 'heading': { const children = block.children.map((child, i) => renderInline(child, `${key}-${i}`, mediaAssets)); const headings = { 1: <h1 key={key}>{children}</h1>, 2: <h2 key={key}>{children}</h2>, 3: <h3 key={key}>{children}</h3>, 4: <h4 key={key}>{children}</h4>, 5: <h5 key={key}>{children}</h5>, 6: <h6 key={key}>{children}</h6> }; return headings[block.level]; }
    case 'blockquote': return <blockquote key={key}>{block.blocks.map((child, i) => renderBlock(child, `${key}-${i}`, mediaAssets))}</blockquote>;
    case 'code-block': return <pre key={key}><code>{block.text}</code></pre>;
    case 'thematic-break': return <hr key={key} />;
    case 'table': return <table key={key}><thead><tr>{block.header.map((cell, i) => <th key={i} scope="col">{cell.map((child, j) => renderInline(child, `${key}-h${i}-${j}`, mediaAssets))}</th>)}</tr></thead><tbody>{block.rows.map((row, i) => <tr key={i}>{row.map((cell, j) => <td key={j}>{cell.map((child, k) => renderInline(child, `${key}-r${i}-${j}-${k}`, mediaAssets))}</td>)}</tr>)}</tbody></table>;
    case 'list': { const items = block.items.map((item, i) => <li key={i}>{item.map((child, j) => renderBlock(child, `${key}-${i}-${j}`, mediaAssets))}</li>); return block.ordered ? <ol key={key}>{items}</ol> : <ul key={key}>{items}</ul>; }
    case 'callout': return <aside key={key} className={`ds-player__callout ds-player__callout--${block.tone}`}><strong>{block.tone}</strong>{block.blocks.map((child, i) => renderBlock(child, `${key}-${i}`, mediaAssets))}</aside>;
    case 'node-embed': return <section key={key} className="ds-player__embed" aria-label="Embedded passage">{block.blocks.map((child, i) => renderBlock(child, `${key}-${i}`, mediaAssets))}</section>;
  }
}

function renderInline(node: SafeInline, key: string, mediaAssets?: MediaAssetApi): ReactNode {
  switch (node.kind) {
    case 'text': return node.text;
    case 'code': return <code key={key}>{node.text}</code>;
    case 'emphasis': return <em key={key}>{node.children.map((child, i) => renderInline(child, `${key}-${i}`, mediaAssets))}</em>;
    case 'strong': return <strong key={key}>{node.children.map((child, i) => renderInline(child, `${key}-${i}`, mediaAssets))}</strong>;
